...
def update_pr_branch(pr_number: int, default_branch: str) -> bool:
  ...
  if not result.success:
    ...
    # Immediately notify the PR creator about the update failure
    notify_pr_creator_of_update_failure(pr_number, default_branch, result.error_details)
    return False
...
def notify_pr_creator_of_update_failure(pr_number: int, default_branch: str, error_details: str = None):
  """Immediately notify the PR creator when branch update fails."""
  print(f"🔔 Notifying PR creator about update failure for PR #{pr_number}...")

  # Get PR author
  author_result = GitHubUtils.get_pr_author(str(pr_number))
  if not author_result.success:
    print(f"⚠️ Could not get author for PR #{pr_number}, skipping notification")
    return
  ...
  failure_message = f"""@{author} ⚠️ **Branch Update Failed**

This PR could not be updated with the latest `{default_branch}` branch during the merge queue process. This typically happens due to:

• **Merge conflicts** that need manual resolution
• **Branch protection rules** preventing the update
• **Permission issues** or other repository constraints

**Next Steps:**
1. Pull the latest changes from `{default_branch}` into your branch
2. Resolve any merge conflicts manually
3. Push the resolved changes to your branch
4. The PR will be ready for the next merge cycle{error_info}

*This comment was automatically generated by the merge queue workflow. The merge queue will continue processing other PRs.*"""

  # Post the comment
  comment_result = GitHubUtils.comment_on_pr(str(pr_number), failure_message)

  if comment_result.success:
    print(f"✅ Successfully notified @{author} about update failure on PR #{pr_number}")
  else:
    print(f"⚠️ Failed to notify PR creator about update failure: {comment_result.error_details}")


def notify_pr_creator_of_ci_failure(pr_number: int, run_id: str, conclusion: str, workflow_name: str):
  """Immediately notify the PR creator when CI fails."""
  print(f"🔔 Notifying PR creator about CI failure for PR #{pr_number}...")
  ...
  # Create failure message
  failure_message = f"""@{author} ❌ **CI Tests Failed**
...
**Next Steps:**
1. Check the [workflow run logs](https://github.com/${{{{ github.repository }}}}/actions/runs/{run_id}) for detailed error information
...
• New code that doesn't meet quality standards
• Flaky tests that need to be stabilized
...
*This comment was automatically generated by the merge queue workflow. The merge queue will continue processing other PRs.*"""
  ...
  if comment_result.success:
    print(f"✅ Successfully notified @{author} about CI failure on PR #{pr_number}")
  else:
    print(f"⚠️ Failed to notify PR creator about CI failure: {comment_result.error_details}")


def notify_pr_creator_of_ci_timeout(pr_number: int, run_id: str, max_wait: int):
  """Immediately notify the PR creator when CI times out."""
  print(f"🔔 Notifying PR creator about CI timeout for PR #{pr_number}...")
  ...
  timeout_message = f"""@{author} ⏰ **CI Tests Timed Out**

The automated tests for this PR have exceeded the maximum wait time ({max_wait_minutes} minutes) during the merge queue process.

**Timeout Details:**
• **Run ID**: {run_id}
• **Max Wait Time**: {max_wait_minutes} minutes
• **Status**: Tests did not complete within the allowed time

**Next Steps:**
1. Check the [workflow run logs](https://github.com/${{{{ github.repository }}}}/actions/runs/{run_id}) to see what was running when it timed out
2. Look for:
   - Tests that are running too slowly
   - Infinite loops or hanging processes
   - Resource constraints or infrastructure issues
3. Optimize slow tests or fix hanging code
4. Push the fixes to your branch
5. The PR will be ready for the next merge cycle

**Common causes of timeouts:**
• Tests that take too long to execute
• Deadlocks or infinite loops in the code
• External service dependencies that are slow/unavailable
• Infrastructure issues with the CI environment

*This comment was automatically generated by the merge queue workflow. The merge queue will continue processing other PRs.*"""

  # Post the comment
  comment_result = GitHubUtils.comment_on_pr(str(pr_number), timeout_message)

  if comment_result.success:
    print(f"✅ Successfully notified @{author} about CI timeout on PR #{pr_number}")
  else:
    print(f"⚠️ Failed to notify PR creator about CI timeout: {comment_result.error_details}")


def notify_pr_creator_of_startup_timeout(pr_number: int, max_wait: int):
  """Immediately notify the PR creator when CI startup times out."""
  print(f"🔔 Notifying PR creator about CI startup timeout for PR #{pr_number}...")
  ...
  # Create startup timeout message
  startup_timeout_message = f"""@{author} ⏰ **CI Startup Timed Out**

The automated tests for this PR failed to start within the expected time ({max_wait_minutes} minutes) during the merge queue process.

**What happened:**
• The merge queue posted an "Ok to test" comment to trigger CI
• The CI workflow did not respond with a "CI job started" comment within {max_wait_minutes} minutes
• This usually indicates an issue with the CI system or workflow configuration

**Next Steps:**
1. Check if there are any issues with the GitHub Actions workflows in this repository
2. Look for workflow configuration problems that might prevent CI from starting
3. Check if there are resource constraints or queuing issues in GitHub Actions
4. If this is a temporary issue, the PR will be ready for the next merge cycle
5. If this persists, contact the repository maintainers

**Common causes of startup timeouts:**
• GitHub Actions experiencing high load or outages
• Workflow configuration errors (syntax, permissions, etc.)
• Missing or misconfigured workflow triggers
• Repository or organization-level restrictions on Actions
• Resource limits or billing issues affecting Actions

*This comment was automatically generated by the merge queue workflow. The merge queue will continue processing other PRs.*"""

  # Post the comment
  comment_result = GitHubUtils.comment_on_pr(str(pr_number), startup_timeout_message)

  if comment_result.success:
    print(f"✅ Successfully notified @{author} about CI startup timeout on PR #{pr_number}")
  else:
    print(f"⚠️ Failed to notify PR creator about CI startup timeout: {comment_result.error_details}")
...
def wait_for_ci_job_started_comment(pr_number: int, trigger_time: str,
  ...
  # Notify the PR creator about startup timeout
  notify_pr_creator_of_startup_timeout(pr_number, max_wait)
  return ""


def wait_for_workflow_run_completion(run_id: str, pr_number: int, max_wait: int,
  ...
  """
    ...
    Args:
        ...
        pr_number: The PR number for notifications
    ...
    Returns:
        ...
        "startup_timeout" - Timed out during startup phase
    """
    ...
    try:
      ...
      if status == "completed":
        if conclusion == "success":
          ...
          # Immediately notify the PR creator about CI failure
          notify_pr_creator_of_ci_failure(pr_number, run_id, conclusion, workflow_name)
          return "failed"
  ...
  # Notify the PR creator about CI timeout
  notify_pr_creator_of_ci_timeout(pr_number, run_id, max_wait)
  return "timeout"
...
def main():
  ...
  for pr_number in pr_numbers:
    ...
    if not update_pr_branch(pr_number, default_branch):
      ...
      # Brief pause to ensure notification is sent before continuing with other PRs
      time.sleep(2)
      continue
    ...
    ci_result = wait_for_workflow_run_completion(run_id, pr_number, max_wait_seconds,
                                                 check_interval)
